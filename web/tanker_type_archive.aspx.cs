...
public partial class tanker_type_archive : System.Web.UI.Page
{
    ...
    {
        DataTable dt = new TankerType().SelectAllArchive();
        ...
        try
        {
            foreach (DataRow row in dt.Rows)
            {
                ...
                sb.AppendLine("         <td style='width: 250px;'> <a href='javascript:;' class='btn btn-info btn-sm btnRestore' id='" + DataHelper.longParse(row["TankerTypeId"]) + "'><i class='glyphicon glyphicon-repeat icon-white'></i> Restore</a> <a href='javascript:;' class='btn btn-danger btn-sm btnDelete' id='" + DataHelper.longParse(row["TankerTypeId"]) + "'><i class='glyphicon glyphicon-trash icon-white'></i> Delete</a> </td>");
                sb.AppendLine("         </tr>");
            }
        }
        catch (Exception ae)
