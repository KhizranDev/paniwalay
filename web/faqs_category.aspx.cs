...
public partial class faqs_category : System.Web.UI.Page
{
    ...
    {
        DataTable dt = new FaqsCategory().SelectAll(0);
        ...
        try
        {
            ...
            {
                sb.AppendLine("         <tr>");
                sb.AppendLine("         <td style='width: 80px;'>" + DataHelper.longParse(row["CategoryId"]) + "</td>");
                ...
                sb.AppendLine("         <td style='width: 250px;'><a href='add_faq_category.aspx?id=" + Encryption.Encrypt(DataHelper.stringParse(row["CategoryId"])) + "&action=" + Encryption.Encrypt("edit") +
                    "' class='btn btn-info btn-sm'><i class='glyphicon glyphicon-edit icon-white'></i> Edit</a>" +
                    " <a href='javascript:;' class='btn btn-danger btn-sm btnDelete' id='" + DataHelper.longParse(row["CategoryId"]) + "'><i class='glyphicon glyphicon-trash icon-white'></i> Delete</a></td>");
                sb.AppendLine("         </tr>");
            }
        }
        catch (Exception ae)
    ...
    {
        if (Session["Category_save"] != null)
        {
            bool sessionValue = DataHelper.boolParse(Session["Category_save"]);
