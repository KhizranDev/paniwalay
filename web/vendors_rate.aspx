<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="vendors_rate.aspx.cs" Inherits="vendors_rate" %>
                    ...
                    <div class="x_content">
                        ...
                        <form id="form1" runat="server" class="form-horizontal form-label-left" autocomplete="off">
                            ...
                            <div class="form-group" id="divVendor" runat="server">
                                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="ddlVendors">Vendor <span class="required">*</span></label>
                                <div class="col-md-4 col-sm-4 col-xs-12">
                                    <asp:DropDownList ID="ddlVendors" data-live-search="true" CssClass="form-control col-md-7 col-xs-12 default-select2" runat="server" DataTextField="VendorName" DataValueField="VendorId" ClientIDMode="Static">
                                    </asp:DropDownList>
                                    <div class="input-error form-control-input" style="color: Red; display: none;">Vendor is required</div>
                                </div>
                            </div>

                            <div class="form-group">
                                <label class="control-label col-md-3 col-sm-3 col-xs-12" for="ddlTankerType">Tanker Type <span class="required">*</span></label>
                                <div class="col-md-4 col-sm-4 col-xs-12">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $ddlTankerType.on('change', function () {
            var vendorid = $("#txtVendorId").val() > 0 ? $("#txtVendorId").val() : $ddlVendors.val();
            LoadRates(vendorid, $(this).val());
        });

        $ddlVendors.on('change', function () {
            LoadRates($(this).val(), $ddlTankerType.val());
        });
        ...
        $(document).on('click', '#btnSave', function () {
            ...
            if (date != '' && rate != '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "RequestHandlerVendor.ashx?action=save_rate",
                    data: {
                        'vendor_id': $ddlVendors.val(),
                        'rate_id': rate_id,
                    ...
                    if (data == '000') {
                        ...
                        LoadRates($ddlVendors.val(), $ddlTankerType.val());
                        clear_values();
    ...
    function LoadRates(vendor_id, tanker_type_id) {
        ...
        $.ajax({
            type: "POST",
            url: "RequestHandlerVendor.ashx?action=get_rates",
            data: {
                "vendor_id": vendor_id,
